    ...
    <body>
        ...
        <script>
            ...
            function getTags(){
                ...
                // Read the users
                $.ajax({
                   url: <cfoutput>"#sDeployURL#/gitinfo.cfm?environment="+environment+"&repo="+repo+"&ts=#getEpoch()#"</cfoutput>,
                   dataType: "json",
            ...
            function checkTags(){
                ...
                if ($('input[name=environment]:checked').val() == "staging" || $('input[name=environment]:checked').val() == "production"){
                    $('[id^=branch-]').hide();
                    $('#branch-live').show();
            ...
            function checkResets(name){
                // Work out if the user has selected a server range
                if (name == 'resets'){
                    bHasChosenServers = true;
                }

                // Only apply auto logic if use has not choosen a server
                if (!bHasChosenServers){
                    // Defaults
                    $('input[name=resets]').prop('checked', false);
                    $('label[group=resets]').removeClass('active');

                    // Hide options for non prod servers
                    if ($('input[name=environment]:checked').val() == 'dev' || $('input[name=environment]:checked').val() == 'sit' || $('input[name=environment]:checked').val() == 'uat'){
                        $('label[group=resets]').hide();
                        $('label[name=resetAll]').show();
                        $('label[name=none]').show();

                    } else {
                        $('label[group=resets]').show();
                    }

                    // Zen staging uses B Servers only
                    if ($('input[name=environment]:checked').val() == 'staging' && $('input[name=repo]:checked').val() == 'zen'){
                        $('label[name=resetB]').addClass('active');
                        $('label[group=resets]').hide();
                        $('label[name=resetB]').show();
                        $('label[name=none]').show();
                        $('#resetB').prop('checked', true);
                    } else {
                        $('label[name=resetAll]').addClass('active');
            ...
            <cfif bDeploy>
                ...
                $('#backBtn').click(function(event) {
                    var url = window.location.protocol + '//' + window.location.host + window.location.pathname + '?type=cf';
                ...
                <div class="cluster">
                    ...
                    <div class="config" >
                            ...
                            <div class="btn-group" data-toggle="buttons">
                                <cfscript>
                                    for (i=1; i lte listLen(lCFEnvironments); i++){
                                        ...
                                        writeOutput('<input class="environment" type="radio" id="environment" name="environment" value=#sEnvironment# #checked#>#uCase(sEnvironment)#</input>');
                                        writeOutput('</label>');
                                    }
                                </cfscript>
                            </div>
                        ...
                        <div class="repo">
                            ...
                            <div class="btn-group" data-toggle="buttons">
                                <cfscript>
                                    lCFRepos = listSort(lCFRepos, "Text");
                                    for (i=1; i lte listLen(lCFRepos); i++){
                                        active = "";
                                        checked = "";
                                        sRepo = listGetAt(lCFRepos, i);
                        ...
                        <div class="reset">
                            <h3>
                                <span class="label label-primary">Resets</span>
                            </h3>
                            <div class="btn-group" data-toggle="buttons" id="resets">
                                <label class="btn btn-default resets active" name="resetAll" group="resets">
                                    <input class="resets" type="radio" id="resetAll" name="resets" value="cfserver" checked="checked">All Servers</input>
                                </label>
                                <label class="btn btn-default resets" name="resetA" group="resets">
                                    <input class="resets" type="radio" id="resetA" name="resets" value="cfserver-a">A Servers</input>
                                </label>
                                <label class="btn btn-default resets" name="resetB" group="resets">
                                    <input class="resets" type="radio" id="resetB" name="resets" value="cfserver-b">B Servers</input>
                                </label>
                                <label class="btn btn-default resets" name="none" group="resets">
                                    <input class="resets" type="radio" id="none" name="resets" value="no-resets">None</input>
                                </label>
                            </div>
                        </div>
                        ...
                        <div class="branch">
                            ...
                            <div class="btn-group" data-toggle="buttons">
                                <cfscript>
                                    //lCFBranches = listSort(lCFBranches, "Text");
                                    for (i=1; i lte listLen(lCFBranches); i++){
                                        active = "";
                                        checked = "";
                                        sBranches = listGetAt(lCFBranches, i);
